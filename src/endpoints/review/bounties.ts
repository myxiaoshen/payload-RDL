...
/** 审核待发布的悬赏任务；仅管理员/审核员可调用。approve -> open，reject -> rejected。 */
export const reviewBountiesEndpoint: Endpoint = {
  path: '/review/bounties',
  method: 'post',
  handler: async (req: PayloadRequest) => {
    ...
    if (target.status !== 'pending') {
      return json({ error: '该悬赏不在待审核状态' }, 400)
    }

    const doc = await req.payload.update({
      collection: 'bounties',
      id,
      data: { status: action === 'approve' ? 'open' : 'rejected' },
      depth: 0,
      overrideAccess: true,
      req,
    })
